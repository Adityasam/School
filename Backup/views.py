...
def get_faculty_remarks(receivercode=""):
    ...
    with connection.cursor() as cursor:
        ...
        left join main_Faculty f on f.FacultyCode = mu.EmployeeCode
...
def load_calendar_events(request):
    ...
    calendar = Calendar.objects.filter(EventDate = eventdate)
...
def get_parent_faculty(request):
    ...
    with connection.cursor() as cursor:
        ...
        inner join main_MyUser mu on mu.EmployeeCode = f.FacultyCode
...
def get_faculty_dashboard(request):
    ...
    with connection.cursor() as cursor:
        ...
        where strftime('%%Y-%%m-%%d', AttendanceDate)=%s and fa.SessionId = %s LIMIT 1) as AttendanceStatus,
...
def get_member_list(request):
    ...
    with connection.cursor() as cursor:
        ...
        where s.Deleted = 0 and s.Status = %s and s.FullName like %s and (%s = '' or %s = s.BatchId)
        and (%s='' or %s=s.CompanyCode)
        order by s.id""",
        [status,"%"+search+"%" ,batch, batch, comp, comp])
...
def get_member_fee_list(request):
    ...
    alldata = []
    with connection.cursor() as cursor:
        cursor.execute("""select f.*, s.FullName as MemberName,
          m.Title as ModeTitle, t.Title as TypeTitle, c.Title as batchTitle, f.DocumentCode as docoumentcode
        from main_Fee f
        left join main_LibraryMember s on s.MemberCode = f.StudentCode
        left join main_Batch c on c.id = s.BatchId
        left join main_PaymentMode m on m.id = f.Mode
        left join main_FeeType t on t.id = f.Type
        where f.Deleted = 0 and s.FullName like %s and f.Type = 'M' and f.Deleted = 0 and
        (%s='' or %s=s.CompanyCode)""", ['%'+search+'%', comp, comp])
...
def get_member_count(request):
    counts = []
    with connection.cursor() as cursor:
        cursor.execute("""select (select COUNT(m.id) from main_LibraryMember m where m.batchid = b.id and m.Active = 1) as BatchCount,
        (select COUNT(m.id) from main_LibraryMember m
        inner join main_Attendance a on m.MemberCode = a.StudentCode
        where m.batchid = b.id and m.Active = 1 and a.Status = 'P' and strftime('%%Y-%%m-%%d', a.AttendanceDate) = %s) as PresentCount,
...
def load_member_graph(request):
    ...
    mems = LibraryMember.objects.filter(Deleted = False, CompanyCode = request.session["COMPANY"])
    cm = int(timezone.now().strftime("%m"))
    year = int(timezone.now().strftime("%Y"))
    for i in range(12):
        newdate = datetime(year, cm, 1, 0, 0, 0).date()
        ...
        for m in mems:
            ...
            if newdate >= joindate and newdate < leavedate:
                ncount += 1
